...
int progress_bar(const char *message, size_t current, size_t total) {
  ...
  if (prev != cur_percentage) {
    printf("%s: %3d%% |%.*s%*s| ", message, cur_percentage, bar_len, bar,
           scale - bar_len, "");
...
bool load_dictionary_resource(Param *param) {
  ...
  //   if (rcfile.empty()) {
  //     ::RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"software\\mecab", 0, KEY_READ,
  //     &hKey); qvres = ::RegQueryValueExW(hKey, L"mecabrc", 0, &vt,
  ...
  //   if (rcfile.empty()) {
  //     ::RegOpenKeyExW(HKEY_CURRENT_USER, L"software\\mecab", 0, KEY_READ,
  //     &hKey); qvres = ::RegQueryValueExW(hKey, L"mecabrc", 0, &vt,
  //                        reinterpret_cast<BYTE *>(v.get()), &size);
  //     ::RegCloseKey(hKey);
  //     if (qvres == ERROR_SUCCESS && vt == REG_SZ) {
